class BrowserProcessConnection {
    constructor(worker) {
        this.worker = worker;
...
BrowserProcessConnection.get = function(identifier) {
    if (!BrowserProcessConnection.registry[identifier]) {
        BrowserProcessConnection.registry[identifier] = new BrowserProcessConnection("js/lmql.web.min.js");
    }
...
function onLMQLReady() {
    initialized = true;
    console.log("loading")
    const appData = {
        "name": "lmql",
        // monaco get editor content
        "app_input": `argmax "Hello[WHO]" from "openai/text-ada-001" where len(WHO) < 10`,
        "app_arguments": {}
    };

    log("Running..." + appData.app_input)
...
window.addEventListener("load", () => {
    api = BrowserProcessConnection.get("default");
    log("Loading...")

    // check local storage for secret otherwise prompt
    let secret = localStorage.getItem("openai-secret");
    if (secret == null) {
        let secret = prompt("Please enter your OpenAI secret", "");
        api.setSecret(secret)
    } else {
        api.setSecret(secret)
    }

    api.addStatusListener((status) => {
        if (status.status == "idle") {
            if (!initialized) onLMQLReady()
        }
    })

    api.addRenderer({
        add_result: (msg) => {
            if (msg.type == "interpreter-head") {
                log(JSON.stringify(msg.data))
                return;
            } else {
                console.log("add result", msg)
            }
        },
        clear_results: () => {
